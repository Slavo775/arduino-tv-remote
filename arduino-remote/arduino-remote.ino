...
const uint8_t ledStripPin = D0;
const uint8_t samsungPin = D1;
const uint8_t magioPin = D0;
const uint8_t redLedPin = D7;
const uint8_t greenLedPin = D6;

remoteLedStrip ledStrip(ledStripPin);
arduinoTvSamsungRemote samsungRemote(samsungPin);
magioRemote tvRemoteMagio(magioPin);
validation val;
...
void config_rest_server_routing() {
    ...
    http_rest_server.on("/deviceStatus", HTTP_GET, []() {http_rest_server.send(200, "text/html", val.sendInformationAboutDevice("Tv Ovladac", "nodeMCU", "remote", "Nahrada ovladacov zariadenia ovladane pomocou webovej apilikacie")); http_rest_server.sendHeader("Access-Control-Allow-Origin","*");});
    ...
    http_rest_server.on("/ledstrip/buttonON", HTTP_GET, []() {ledStrip.ONButton(); http_rest_server.send(200, "text/html","OK"); http_rest_server.sendHeader("Access-Control-Allow-Origin","*");});
    http_rest_server.on("/ledstrip/buttonOFF", HTTP_GET, []() {ledStrip.OFFButton(); http_rest_server.send(200, "text/html","OK"); http_rest_server.sendHeader("Access-Control-Allow-Origin","*");});
    http_rest_server.on("/ledstrip/brighntessDown", HTTP_GET, []() {ledStrip.brightnessDown(); http_rest_server.send(200, "text/html","OK"); http_rest_server.sendHeader("Access-Control-Allow-Origin","*");});
    http_rest_server.on("/ledstrip/brighntessUp", HTTP_GET, []() {ledStrip.brightnessUp(); http_rest_server.send(200, "text/html","OK"); http_rest_server.sendHeader("Access-Control-Allow-Origin","*");});
    http_rest_server.on("/ledstrip/buttonOne", HTTP_GET, []() {ledStrip.buttonRed(); http_rest_server.send(200, "text/html","OK"); http_rest_server.sendHeader("Access-Control-Allow-Origin","*");});
    http_rest_server.on("/ledstrip/buttonTwo", HTTP_GET, []() {ledStrip.buttonGreen(); http_rest_server.send(200, "text/html","OK"); http_rest_server.sendHeader("Access-Control-Allow-Origin","*");});
    http_rest_server.on("/ledstrip/buttonThree", HTTP_GET, []() {ledStrip.buttonBlue(); http_rest_server.send(200, "text/html","OK"); http_rest_server.sendHeader("Access-Control-Allow-Origin","*");});
}
